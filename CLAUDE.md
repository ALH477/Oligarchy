...
All are deliberately NOT in `checks` (they are slow — the last four need KVM — and `nix flake check` already builds the toplevel). They are separate `packages` outputs.
