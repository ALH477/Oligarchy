...
All are deliberately NOT in `checks` (they are slow — the last three need KVM — and `nix flake check` already builds the toplevel). They are separate `packages` outputs.
