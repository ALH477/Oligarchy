    ...
    flake-utils.lib.eachDefaultSystem
      (system:
        let
          ...
          # Must agree with `allowlist::ASPECTS`, `umbrella::is_known_aspect`,
          # `nixos-module.nix`'s aspectNames, and the repo-root `.mcp.json`.
          aspectNames = [
            "system"
            "net"
            "dcf"
          ...
          mcpSelfAudit = pkgs.runCommand "oligarchy-mcp-self-audit"
            ...
            ''
              mkdir -p $out
              # CARGO_MANIFEST_DIR is baked in at compile time and points at a
              # build sandbox that no longer exists here, so the audit would
              # scan nothing. Hand it the real source tree.
              export OLIGARCHY_MCP_WORKSPACE=${./.}
