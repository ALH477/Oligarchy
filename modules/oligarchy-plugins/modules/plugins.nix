...
let
  ...
  # that needs auditing.
  grantsWx = p: p.jit == "self" && (p.tier == "native" || p.tier == "lua");
    ...
    "oligarchy-plugin@${name}.service" = {
      ...
      text = ''
        ...
        ${optionalString (p.tier == "wasm") ''
          # tier=wasm: MDWE is off because THIS process runs Cranelift. The guest
          # is confined by wasmtime, not by seccomp; setting MDWE here would break
          # compilation without hardening anything.
        ''}
    ...
    (mkMerge ([
      {
        ...
        assertions = [
          ...
          {
            assertion = (builtins.elem "native" cfg.allowedTiers
              || builtins.elem "lua" cfg.allowedTiers)
        ...
        systemd.services.oligarchy-plugind = {
          ...
          path = [ pkgs.nix pkgs.systemd ]
            ++ optional
            (builtins.elem "native" cfg.allowedTiers
              || builtins.elem "lua" cfg.allowedTiers)
            pkgs.bubblewrap;
        ...
        systemd.services."oligarchy-plugin@" = {
          ...
          path = optionals
            (builtins.elem "native" cfg.allowedTiers
              || builtins.elem "lua" cfg.allowedTiers) [ pkgs.bubblewrap ];
          ...
          serviceConfig = {
            ...
            ProtectClock = true;
            RestrictSUIDSGID = true;
            RestrictNamespaces = "user"; # bwrap needs userns and nothing else
            LockPersonality = true;
            SystemCallArchitectures = "native";
            UMask = "0077";
            TasksMax = 64;
