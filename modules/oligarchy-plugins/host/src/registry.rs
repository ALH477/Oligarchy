...
impl Store {
    ...
    fn flush(&self) -> Result<()> {
        // Write-fsync-rename so a crash mid-write cannot leave a registry that
        // parses as "no plugins are confined" — and a power cut after rename
        // cannot leave a zero-length registry.json behind either (rename alone
        // does not guarantee the data reached the disk).
        ...
        let tmp = path.with_extension(format!("json.tmp.{}", std::process::id()));
        {
            ...
            use std::io::Write;
            f.write_all(&serde_json::to_vec_pretty(&self.reg)?)?;
            f.sync_all()?;
        }
        std::fs::rename(&tmp, &path)?;
        if let Ok(dir) = std::fs::File::open(path.parent().unwrap_or(Path::new("/"))) {
            let _ = dir.sync_all();
        }
        Ok(())
    }
    ...
    /// `declared_ids` is the set of ids pinned by custom.plugins.declaredPlugins
    /// (from declared.json), or None when the caller cannot supply it. An
    /// imperative install whose id collides with a declared one would shadow
    /// it in run_one — the unit then runs the imperative artifact under the
    /// drop-in generated for the DECLARED manifest's tier/jit, a sandbox
    /// computed for a different plugin. Refuse instead.
    pub fn install(
        &mut self,
        source: &str,
        policy: &Policy,
        who: Authority,
        declared_ids: Option<&std::collections::BTreeSet<String>>,
    ) -> Result<String> {
        ...
        if let Some(ids) = declared_ids {
            if ids.contains(&manifest.id) {
                bail!(
                    "{} is pinned by custom.plugins.declaredPlugins on this host. \
                     An imperative install would shadow it — the unit would run \
                     this artifact under a sandbox drop-in generated for the \
                     declared manifest. Remove the declaration, reinstall, or \
                     rebuild against this artifact; do not carry both.",
                    manifest.id
                );
            }
        }

        // Reinstalling an enabled plugin used to reset it to enabled=false
        // WHILE leaving the old drop-in in /run and possibly a running unit
        // under the old manifest — registry and /run disagreeing about a W^X
        // grant. Disable first, exactly like remove() does.
        if let Some(existing) = self.reg.entries.get(&manifest.id) {
            if existing.enabled {
...
fn write_dropin(m: &Manifest, state_dir: &Path) -> Result<()> {
    ...
    // MIRROR of mkDropin in modules/plugins.nix. That one serves declared
    // plugins into /etc; this one serves imperative ones into /run. Every
    // directive has to appear in both — NotifyAccess and the AF_NETLINK
    // branch each previously reached only this side, which nothing caught
    // because checks.wx-enforcement only asserts the MDWE line.
    if m.tier == Tier::Microvm {
        // Declared microvm plugins get this block from mkDropin; keep the two
        // generators in lockstep even if imperative tier-2 is ever allowed to
        // load, so the guest's lifecycle wiring cannot silently go missing.
        s.push_str(&format!(
            "[Unit]\nRequires=microvm@{0}.service\nAfter=microvm@{0}.service\n\
             PropagatesStopTo=microvm@{0}.service\n",
            m.id
        ));
    }
    s.push_str("[Service]\n");
