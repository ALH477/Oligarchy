...
let
  ...
  dspImage = archibaldos.packages.${pkgs.system}.dsp;
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    # Combine all kernel parameters into a single mergeable list
    ...
    systemd.services.archibaldos-dsp-vm = {
      ...
      serviceConfig = {
        ...
        ExecStart = ''${pkgs.qemu_kvm}/bin/qemu-system-x86_64 \
          ...
          -cpu host \
          -machine q35 \
          -drive file=${diskImage}/disk.img,format=raw,if=virtio,cache=unsafe \
